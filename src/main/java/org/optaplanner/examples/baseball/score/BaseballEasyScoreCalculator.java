...
public class BaseballEasyScoreCalculator
    implements EasyScoreCalculator<BaseballSolution, BendableLongScore> {
  ...
  public BendableLongScore calculateScore(BaseballSolution baseballSolution) {
    ...
    return BendableLongScore.of(
        new long[] {duplicationHardScore, successiveHardScore},
        new long[] {
          minimizeShortScore,
          stabilizeHolidayScore,
          stabilizeDistanceScore,
          -sumDistance.longValue()
        });
  }
  ...
  private TreeMap<Calendar, List<Match>> getCalendarListTreeMap(BaseballSolution baseballSolution) {
    ...
    for (Match match : baseballSolution.getMatchList()) {
      ...
      if (calendar == null) {
        calendar =
            baseballSolution.getCalendarList().get(baseballSolution.getCalendarList().size() - 1);
      }
